...
istream& dealer::set(istream& in){
    ...
    if (&in == &cin){
        int size;
        cout<<"”кажите длину каталога:\n";
        cin >> size;
        cout<<"¬ведите "<<size<<" авто - марку, модель, скорость и тип трансмиссии:\n";
        car temp;
...
ostream& dealer::get(ostream& out){
    if (&out == &cout){
        cout<<" аталог автомобилей:\n";
        cout<<"јвтоматическа€ трансмисси€:\n";
    }
    for (int i = 0; i < catalogA.size(); i++){
        out<<catalogA[i]<<"\n";
    }
    if (&out == &cout)
        cout<<"ћеханическа€ трансмисси€:\n";
